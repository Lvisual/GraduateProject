...
selectLabelForm::selectLabelForm(QWidget *parent) : QWidget(parent)
{
    ...
    idEdit->setPlaceholderText(QString::fromLocal8Bit("请输入标签ID"));

    formlayout->addRow(QString::fromLocal8Bit("&标签ID："),idEdit);
    ...
    title=new FormTitle(this,QString::fromLocal8Bit("标签查询"));
...
selectLabelForm::~selectLabelForm()
{
}

/********************窗口缩放***********************/

void selectLabelForm::mousePressEvent(QMouseEvent *eve)
{
...
void selectLabelForm::mouseReleaseEvent(QMouseEvent *eve)
{
    if(isLeftPressed)
        isLeftPressed=false;
...
void selectLabelForm::mouseMoveEvent(QMouseEvent *eve)
{
...
int selectLabelForm::countFlag(QPoint p, int row)
{
    if(p.y()<MARGIN)
        return 10+row;
...
void selectLabelForm::setCursorType(int flag)
{
...
int selectLabelForm::countRow(QPoint p)
{
