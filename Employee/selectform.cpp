...
selectForm::selectForm(QWidget *parent) : QWidget(parent)
{
...
void selectForm::combine(){
    int id;
    id = this->idEdit->text().toInt();
    emit newEmployeeId(id);
    this->close();
}
...
void selectForm::mousePressEvent(QMouseEvent *eve)
{
...
void selectForm::mouseReleaseEvent(QMouseEvent *eve)
{
    if(isLeftPressed)
        isLeftPressed=false;
...
void selectForm::mouseMoveEvent(QMouseEvent *eve)
{
...
int selectForm::countFlag(QPoint p, int row)
{
    if(p.y()<MARGIN)
        return 10+row;
...
void selectForm::setCursorType(int flag)
{
...
int selectForm::countRow(QPoint p)
{
