...
#include <QFont>
#include <QDebug>
baseTypeForm::baseTypeForm(QWidget *parent) :
    QWidget(parent)

{
   ...
   // formlayout->addRow(QStringLiteral("&基站类型ID："),idEdit);
    ...
    title=new FormTitle(this,QStringLiteral("基站类型"));
...
void baseTypeForm::combine(){
    baseTypeInfo newbaseTypeInfo;
    newbaseTypeInfo.setBaseTypeId(this->idEdit->text());
    newbaseTypeInfo.setBaseTypeName(this->typenameEdit->text());
    newbaseTypeInfo.setBaseTypeRemark(this->remarkEdit->text());
    emit newBaseTypeItem(newbaseTypeInfo);
    this->close();
}

baseTypeForm::~baseTypeForm()
{
}

/********************窗口缩放***********************/

void baseTypeForm::mousePressEvent(QMouseEvent *eve)
{
...
void baseTypeForm::mouseReleaseEvent(QMouseEvent *eve)
{
    if(isLeftPressed)
        isLeftPressed=false;
...
void baseTypeForm::mouseMoveEvent(QMouseEvent *eve)
{
...
int baseTypeForm::countFlag(QPoint p, int row)
{
    if(p.y()<MARGIN)
        return 10+row;
...
void baseTypeForm::setCursorType(int flag)
{
...
int baseTypeForm::countRow(QPoint p)
{
