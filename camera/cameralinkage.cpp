    ...
    ui(new Ui::CameraLinkage)
{
    ...
    xuanze->setText(QStringLiteral("选择地图模型"));
    shanchu=new QPushButton();
    shanchu->setText(QStringLiteral("刷新"));
...
//    m_naviView->setItemDelegate(delegate);
    ...
    connect(xuanze , SIGNAL(clicked(bool)),this, SLOT(setMap()));
...
void CameraLinkage::setMap(){
     QString s = QFileDialog::getOpenFileName(0, "Select A File", "/", "Txt File(*.3ds)");
}
